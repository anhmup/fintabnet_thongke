ma# %% [markdown]
# make data for merge using fintabnet
...
# %%
data_fintabnet_path =  "D:/fintabnet.tar/fintabnet/"
subdir = "train"
...
# %%
save_path_matrix_col = 'D:/learn/LV/merge/data/col/'
save_path_matrix_row = 'D:/learn/LV/merge/data/row/'
save_path_image = 'D:/learn/LV/merge/data/image/'
...
for i in range(0,99999):
    ...
    try:
        ...
        reshape_image , cell_bbox_512 = change_bbox_to_new_size(np.array(image_table_crop), bbox_cell_ori , size = 512) 
        if (len(cell_bbox_512) == (final_matrix_col.shape[0] * final_matrix_col.shape[1])) and (len(cell_bbox_512) == (final_matrix_row.shape[0] * final_matrix_row.shape[1])):
            np.save(save_path_matrix_col + file_name_str + '.npy', final_matrix_col)
            np.save(save_path_matrix_row + file_name_str + '.npy', final_matrix_row)

            image_table_crop.save(save_path_image + file_name_str + '.jpg')

            update_name = file_name_str
            update_bbox = [cell_bbox for cell_bbox in cell_bbox_512]
            update_all = {update_name: update_bbox}
            data_frame.update(**update_all)
        else:
            print(file_name_str)
    except:
        data_detection = json.loads(structure_lines[i])
...
# %%
import pandas as pd
df = pd.DataFrame(data_frame.items())
df.to_csv('fintab_merge_data_train_full.csv' , index= False)
...
# %%
matrix_col_path  = 'D:/learn/LV/merge/data/col/'
matrix_row_path  = 'D:/learn/LV/merge/data/row/'
image_path =       'D:/learn/LV/merge/data/image/'
...
data_test = CustomDataset(df_test)
...
# %%
data_test = CustomDataset(df_test[60000:])
data_loader = DataLoader(data_test, batch_size= 1, shuffle=False )
...
# %%
row_np = output_['row'].numpy()

# %%
row_np_reshape = np.reshape(row_np, (8,3))
row_np_reshape
